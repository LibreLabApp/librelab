...
/// Generated mapping for the `storage_objects` table, providing type-safe references
/// for the table name and column names to avoid hardcoding strings.
abstract final class StorageObjectsTable {
  static const String tableName = 'storage_objects';
...
/// Generated row mapping for the `storage_objects` table.
/// Represents a full-row result where all columns are expected to be present.
///
/// This model assumes SELECT queries include all columns defined in the table (i.e., `SELECT * FROM storage_objects`).
/// Partial SELECT projections are **not** supported and may result in runtime errors.
@immutable
final class StorageObjectsRow {
  const StorageObjectsRow({
    required this.id,
